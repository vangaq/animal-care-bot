...
def create_pet_sync(user_id: int, breed: str, name: str, age: str, extra_info: str | None = None):
...
def update_pet_field_sync(pet_id: int, field: str, value: str):
    allowed_fields = {"breed", "name", "age", "extra_info"}
    ...
    try:
        ...
        if field == "name":
            conflict = (
                session.query(Pet)
                .filter(Pet.user_id == pet.user_id, Pet.name == value, Pet.id != pet.id)
                .first()
            )
            if conflict:
                return make_response_error("Питомец с такой кличкой уже существует.")
...
def create_note_sync(pet_id: int, title: str, period: str, extra_info: str | None = None):
    ...
    if period not in valid_periods:
        return make_response_error(
            "Неправильная периодичность. Выберите одну из: " + ", ".join(valid_periods)
        )
...
def update_note_field_sync(note_id: int, field: str, value: str):
    allowed_fields = {"title", "period", "extra_info"}
    if field not in allowed_fields:
...
async def create_pet(user_id: int, breed: str, name: str, age: str, extra_info: str | None = None):
...
async def create_note(pet_id: int, title: str, period: str, extra_info: str | None = None):
